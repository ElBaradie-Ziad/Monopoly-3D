...
# ────────────────────────────────────────
# Fonction principale : on passe uniquement l'ID de la carte Chance
func show_card(card_id: int) -> void:
	var card = GameData.CHANCE_CARDS.get(card_id)
...
func _play_card_effect(type: String) -> void:
	match type:
		"positive":
			# Célébration verte : flash + rebond
			var tw = create_tween().set_parallel(true)
			tw.tween_property(self, "modulate", Color(0.6, 1.5, 0.5, 1.0), 0.15)
		...
		"special":
			# Lueur cyan pulsée (carte à conserver)
			var tw = create_tween().set_loops(2)
			tw.tween_property(self, "modulate", Color(0.4, 1.2, 1.3, 1.0), 0.25)
...
func _ready() -> void:
	hide()
	#show_card(101)   # Test avec la première carte Chance

 ...
 * std::declval<EProperties>()...)</tt> is well formed; otherwise @c false_type.
 */
template <typename T, typename... EProperties>
struct can_require :
 ...
 * @c true_type if the expression <tt>asio::require(std::declval<T>(),
 * std::declval<EProperties>()...)</tt> is @c noexcept; otherwise @c false_type.
 */
template <typename T, typename... EProperties>
...
template <typename Impl, typename T, typename EProperties, typename = void,
...
struct can_require :
  integral_constant<bool,
    asio_require_fn::call_traits<
      require_t, T, void(EProperties...)>::overload
...
template <typename T, typename ASIO_ELLIPSIS EProperties>
constexpr bool can_require_v
  = can_require<T, EProperties ASIO_ELLIPSIS>::value;
...
struct is_nothrow_require :
  integral_constant<bool,
    asio_require_fn::call_traits<
      require_t, T, void(EProperties...)>::is_noexcept>
{
};
...
constexpr bool is_nothrow_require_v
  = is_nothrow_require<T, EProperties ASIO_ELLIPSIS>::value;
...
struct require_result
{
  typedef typename asio_require_fn::call_traits<
      require_t, T, void(EProperties...)>::result_type type;
};
